...
def set_picker_value(driver, picker_locator, value):
    ...
    try:
        ...
        while current_value != value:
            ...
            time.sleep(1)
            current_value = picker.text  # Recheck the value after swiping

        logging.info(f"Picker value set to: {value}")
    except Exception as e:
        logging.error(f"Failed to set picker value to: {value}, Error: {e}")
        raise
...
def select_full_date(driver):
    ...
    random_day = str(random.randint(1, 30))  # Adjust as needed for the actual day range
    ...
    # Validate the date picker has updated correctly
    selected_day = driver.find_element(*date_picker).text
    selected_month = driver.find_element(*month_picker).text
    selected_year = driver.find_element(*year_picker).text

    if selected_day == random_day and selected_month == random_month and selected_year == random_year:
        wait_and_click(driver, OK_BUTTON)
        print("Date Selection Confirmed!")
    else:
        raise Exception("Date picker values not updated correctly")
...
try:
    ...
    PHONE_FIELD = (By.XPATH, '//android.widget.EditText[@text="Phone number"]')
