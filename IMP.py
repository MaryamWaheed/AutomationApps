...
def scroll_to_value_and_select(driver, picker_locator, value):
    """
    Scroll to the desired value in a NumberPicker and select it by clicking.
    """
    try:
        picker = WebDriverWait(driver, 20).until(EC.presence_of_element_located(picker_locator))

        # Get picker size and position
        start_x = picker.location['x'] + picker.size['width'] // 2
        start_y = picker.location['y'] + picker.size['height'] - 10
        end_y = picker.location['y'] + 10  # Swipe up

        # Define actions for swipe gestures
        actions = ActionBuilder(driver)
        pointer = PointerInput("touch", "finger")

        # Attempt to scroll and find the desired value
        for _ in range(10):  # Limit the number of swipes
            visible_values = picker.find_elements(By.CLASS_NAME, 'android.widget.EditText')
            ...
            actions.add_action(
                pointer.create_pointer_move(duration=500, x=start_x, y=end_y, origin='viewport')
            )
            actions.add_action(pointer.create_pointer_up(driver))
            actions.perform()

            time.sleep(1)  # Small delay after swipe

        logging.error(f"Failed to find the value: {value} in picker: {picker_locator}")
        raise Exception(f"Value {value} not found in the picker")
    except Exception as e:
...
def select_full_date(driver):
    """Select the date by scrolling and setting date, month, and year, and then clicking OK."""
    ...
    scroll_to_value_and_select(driver, date_picker, "03")
    print("Day Set!")
    time.sleep(2)

    logging.info("Setting Month...")
    scroll_to_value_and_select(driver, month_picker, "Jul")
    print("Month Set!")
    time.sleep(2)

    logging.info("Setting Year...")
    scroll_to_value_and_select(driver, year_picker, "2021")
    print("Year Set!")
    time.sleep(2)
...
def main():
    ...
    try:
        ...
        SIGNUP1_BUTTON = (By.ANDROID_UIAUTOMATOR, 'new UiSelector().text("Signup")')
